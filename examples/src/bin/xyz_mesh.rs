...
fn rcb<'a>(mesh: impl Mesh<Dim = D3>, matches: &ArgMatches<'a>) {
    ...
    let partition = algorithms::recursive_bisection::rcb_2d(&weights, &points_clone, num_iter);
    let end = now.elapsed();
...
fn rib<'a>(mesh: impl Mesh<Dim = D3>, matches: &ArgMatches<'a>) {
    ...
    let partition = algorithms::recursive_bisection::rib_2d(&weights, &points.clone(), num_iter);
