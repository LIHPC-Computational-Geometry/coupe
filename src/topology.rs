    ...
    T: Copy,
{
    adjacency
        .into_iter()
        .filter(|(_edge_weight, (node1, node2))| {
            node1 < node2 && partition[*node1] != partition[*node2]
        })
        .map(|(edge_weight, (node1, node2))| (node1, node2, *edge_weight))
}
...
where
    T: Copy + Sum,
{
    edge_cut(&adjacency, partition)
        .map(|(_n1, _n2, edge_weight)| edge_weight)
        .sum()
}
