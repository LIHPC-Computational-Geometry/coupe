...
type PartId = usize;
type BoxIndex = [usize; NB_CRITERIA];
// type BixId =
    ...
    W: Weight,
{
    ...
    // Mapping used to search for weights with specific weights
    pub boxes: HashMap<BoxIndex, Vec<Vec<W>>>,
}
    ...
    W: Weight,
{
    ...
    fn new(cweights: &'a Vec<Vec<W>>, nb_intervals: Vec<usize>) -> Self {
        let mut boxes: HashMap<BoxIndex, Vec<Vec<W>>> = HashMap::new();
        if cweights.len() <= 1 {
        ...
        cweights.iter().enumerate().for_each(|(id, cweight)| {});
        // for (pos, cweight) in cweights.iter().enumerate() {
    ...
    fn indices(
        cweight: [W; NB_CRITERIA],
        min_weights: [W; NB_CRITERIA],
        max_weights: [W; NB_CRITERIA],
    ) {
        let mut out: BoxIndex = [0; NB_CRITERIA];
        cweight
            .iter()
            .enumerate()
            .for_each(|(criterion, weight)| {});
...
// impl<'a, T> BoxHandler<'a, T>
// where
//     T: Weight,
// {
//     fn new() -> Self {
//         let mut boxes: HashMap<[; NB_CRITERIA], Vec<usize>> = HashMap::new();
...
mod tests {
    ...
    fn check_regular_box_handler() {
        ...
        assert_eq!(rbh.min_weights[0], 1);
        assert_eq!(rbh.min_weights[1], 1);
        assert_eq!(rbh.max_weights[0], 4);
        assert_eq!(rbh.max_weights[1], 5);

        let indices = compute_box_indices(
            &instance.cweights[0],
            rbh.min_weights,
            rbh.max_weights,
            rbh.nb_intervals,
        ...
        assert!(
            indices.iter().zip([0, 0].iter()).all(|(a, b)| a == b),
