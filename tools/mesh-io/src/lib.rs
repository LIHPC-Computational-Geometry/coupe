...
/// Parse error.
///
/// The error type is marked as `#[non_exhaustive]` to ease changes of cargo
/// features.
#[derive(Debug)]
#[non_exhaustive]
pub enum Error {
...
impl From<medit::ParseError> for Error {
    fn from(err: medit::ParseError) -> Self {
        Self::Medit(err)
    }
}
